Feature: Show test accurately for queried vehicle - CVSB-10283
  ...
  AC7 - Return to the Tech Record screen
    ...
    Then tech record fields should have values
      | Field                            | Value     |
      | testResult-tRes-0-tType-0        | PASS      |
      | testCode-tRes-0-tType-0          | Lev       |
    ...
    And I should see "Open all"
    And all test record sections should be collapsed
  ...
  AC5 - System does not display Emission details section if the test type is not a lec one
    ...
    Then tech record fields should have values
      | Field                     | Value     |
      | testResult-tRes-0-tType-0 | ABANDONED |
      | testCode-tRes-0-tType-0   | Ruv       |
    When I go to view test record with index 1
    Then wait until I see "Change test record"
    And test record sections are displayed
      | Section                     |
    ...
    And test record fields should have values
      ...
      | deficiencyRef-0              | deficiency reference (1)*               |
      ...
      | deficencyId-0                | b d deficiency text 1                   |
      | location-0                   | -                                       |
      | defect-prs-0                 | Yes                                     |
      | prohibition-0                | Yes                                     |
      | additionalInfo-0             | -                                       |
      | deficiencyRef-1              | deficiency reference 2                  |
      ...
      | deficencyId-1                | a m deficiency text 2                   |
...
#      | certificateNumber      |
  ...
  AC5 - System does not display Emission details section if the test type is not a lec one
    ...
    And I create test record with status "submitted" and result "fail" and test type "ruv" for previously created vehicle
    When I go to search tech record page
    Then I should see "Vehicle registration mark, trailer ID or vehicle identification number"
    And search vehicle input field should be present
    When I search for previously created vehicle
    ...
    And all test record sections should be expanded
    And I should see "Vehicle identification number (VIN)" in "Vehicle" test record section
    And I should not see "Vehicle registration mark (VRM)" in "Vehicle" test record section
...
#      | certificateNumber      |
    And test record fields should have values
      ...
      | deficencyId-0                | c x deficiency text 1             |
      | location-0                   | Inner/Front                       |
      | defect-prs-0                 | No                                |
      | prohibition-0                | No                                |
      ...
      | deficencyId-1                | d v deficiency text 2             |
      | location-1                   | Upper/Centre/1/2                  |
      | defect-prs-1                 | Yes                               |
      | prohibition-1                | Yes                               |
      | additionalInfo-1             | -                                 |
